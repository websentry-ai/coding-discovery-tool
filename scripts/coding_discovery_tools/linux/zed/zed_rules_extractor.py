"""Zed config extraction for Linux systems.

Paths are identical to macOS (``~/.config/zed`` global, ``<project>/.zed``
project, project-root ``.rules``), so only three OS seams are overridden: the
user-home enumeration, the Linux-aware project walk, and the user-level-dir
test. The gates and allow-lists stay in the shared macOS body.
"""
...
class LinuxZedRulesExtractor(MacOSZedRulesExtractor):
    """Extractor for Zed config on Linux systems."""
