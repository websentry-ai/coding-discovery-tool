"""
OpenAI Codex skills extraction for macOS systems.
...
class MacOSCodexSkillsExtractor(BaseCodexSkillsExtractor):
    """Extractor for Codex skills on macOS systems."""

    def extract_all_skills(self) -> Dict:
        """
        Extract all Codex skills from all projects on macOS.
    ...
    def _extract_project_level_skills(self, root_path: Path, projects_by_root: Dict[str, List[Dict]]) -> None:
        ...
        if root_path == Path("/"):
            try:
                ...
                logger.info("Falling back to home directory search for Codex skills")
                home_path = Path.home()
