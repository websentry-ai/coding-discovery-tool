"""
Gemini CLI skills extraction for macOS systems.
...
class MacOSGeminiCliSkillsExtractor(BaseGeminiCliSkillsExtractor):
    """Extractor for Gemini CLI skills on macOS systems."""

    def extract_all_skills(self) -> Dict:
        """
        Extract all Gemini CLI skills from all projects on macOS.
    ...
    def _extract_project_level_skills(self, root_path: Path, projects_by_root: Dict[str, List[Dict]]) -> None:
        ...
        if root_path == Path("/"):
            try:
                ...
                logger.info("Falling back to home directory search for Gemini CLI skills")
                home_path = Path.home()
