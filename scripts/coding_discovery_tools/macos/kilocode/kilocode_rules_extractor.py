        ...
        if root_path == Path("/"):
            try:
                ...
                for top_dir in top_level_dirs:
                    try:
                        self._walk_for_kilocode_directories(root_path, top_dir, projects_by_root, current_depth=1)
            ...
            for kilocode_dir in root_path.rglob(".kilocode"):
                try:
                    if not should_process_directory(kilocode_dir, root_path):
                        continue

                    # Extract rules from this .kilocode directory
                    self._extract_rules_from_kilocode_directory(kilocode_dir, projects_by_root)
                except (PermissionError, OSError) as e:
