...
class MacOSKiloCodeMCPConfigExtractor(BaseMCPConfigExtractor):
    ...
    def _read_global_config(self, config_path: Path, ide_name: str) -> Optional[Dict]:
        ...
        Args:
            ...
            ide_name: Name of the IDE (Code, Cursor)
        ...
        try:
            ...
            logger.warning(f"Invalid JSON in global Kilo Code MCP config {config_path}: {e}")
        except PermissionError as e:
            logger.warning(f"Permission denied reading global Kilo Code MCP config {config_path}: {e}")
        except Exception as e:
            logger.warning(f"Error reading global Kilo Code MCP config {config_path}: {e}")
    ...
    def _extract_project_level_configs(self) -> List[Dict]:
        ...
        try:
            ...
            # No global .kilocode directory to skip
            ...
            for kilocode_dir in home_path.rglob(".kilocode"):
                try:
                    if not should_process_directory(kilocode_dir, home_path):
                        continue
