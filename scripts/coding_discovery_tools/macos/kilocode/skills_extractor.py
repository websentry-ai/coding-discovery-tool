"""
Kilo Code skills extraction for macOS systems.
...
class MacOSKiloCodeSkillsExtractor(BaseKiloCodeSkillsExtractor):
    """Extractor for Kilo Code skills on macOS systems."""

    def extract_all_skills(self) -> Dict:
        """
        Extract all Kilo Code skills from all projects on macOS.
    ...
    def _extract_project_level_skills(self, root_path: Path, projects_by_root: Dict[str, List[Dict]]) -> None:
        ...
        if root_path == Path("/"):
            try:
                ...
                logger.info("Falling back to home directory search for Kilo Code skills")
                home_path = Path.home()
