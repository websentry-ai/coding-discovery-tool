"""
OpenCode skills extraction for macOS systems.
...
class MacOSOpenCodeSkillsExtractor(BaseOpenCodeSkillsExtractor):
    """Extractor for OpenCode skills on macOS systems."""
    ...
    def _extract_project_level_skills(self, root_path: Path, projects_by_root: Dict[str, List[Dict]]) -> None:
        if root_path == Path("/"):
            try:
                ...
                logger.info("Falling back to home directory search for OpenCode skills")
                home_path = Path.home()
