...
class MacOSRooDetector(BaseToolDetector):
    ...
    # Editor key (vscode_extension_helpers) -> display name.
    SUPPORTED_IDES = {
        'Code': 'VS Code',
        'Cursor': 'Cursor',
        'Windsurf': 'Windsurf',
    ...
    def _detect_roo_for_user(self, user_home: Path) -> List[Dict]:
        ...
        # Gate on the extensions.json entry alone — no host-.app AND-gate, since the
        # entry is itself proof of a live install.
        ...
        # Antigravity keeps its own .app gate (not a marketplace VS Code editor);
    ...
    def _check_ide_installation(
        ...
        Checking both avoids a false negative that would hide a real Roo user
