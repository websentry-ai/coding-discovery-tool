"""
Windsurf (Cascade) skills extraction for macOS systems.
...
class MacOSWindsurfSkillsExtractor(BaseWindsurfSkillsExtractor):
    """Extractor for Windsurf skills on macOS systems."""
    ...
    def _extract_project_level_skills(self, root_path: Path, projects_by_root: Dict[str, List[Dict]]) -> None:
        if root_path == Path("/"):
            try:
                ...
                logger.info("Falling back to home directory search for Windsurf skills")
                home_path = Path.home()
