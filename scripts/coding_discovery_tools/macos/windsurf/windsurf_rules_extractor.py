"""
Windsurf rules extraction for macOS systems.
...
class MacOSWindsurfRulesExtractor(BaseWindsurfRulesExtractor):
    """Extractor for Windsurf rules on macOS systems."""

    def extract_all_windsurf_rules(self) -> List[Dict]:
        """
        Extract all Windsurf rules from all projects on macOS.
        ...
        if root_path == Path("/"):
            try:
                ...
                for top_dir in top_level_dirs:
                    try:
                        self._walk_for_windsurf_directories(root_path, top_dir, projects_by_root, current_depth=1)
            ...
            for windsurf_dir in root_path.rglob(".windsurf"):
                try:
                    if not should_process_directory(windsurf_dir, root_path):
                        continue

                    # Extract rules from this .windsurf directory
                    self._extract_rules_from_windsurf_directory(windsurf_dir, projects_by_root)
                except (PermissionError, OSError) as e:
