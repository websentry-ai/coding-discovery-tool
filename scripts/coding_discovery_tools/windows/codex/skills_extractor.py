"""
OpenAI Codex skills extraction for Windows systems.

Extracts Codex skills from all projects, grouping them by project root.
...
from ...coding_tool_base import BaseCodexSkillsExtractor
...
from ...codex_skills_helpers import (
...
class WindowsCodexSkillsExtractor(BaseCodexSkillsExtractor):
    """Extractor for Codex skills on Windows systems."""
    ...
    def extract_all_skills(self) -> Dict:
        """
        Extract all Codex skills from all projects on Windows.
        ...
        logger.info(f"Searching for Codex skills from root: {root_path}")
    ...
    def _extract_user_level_skills(self, user_skills: List[Dict]) -> None:
        ...
        def extract_for_user(user_home: Path) -> None:
            extract_codex_user_level_items(user_home, user_skills, extract_single_rule_file, CODEX_ITEM_CONFIGS)
            ...
            for item in current_dir.iterdir():
                try:
                    if (
                        ...
                        or traverses_other_tool_config_dir(item, allow=SHARED_SKILL_DIRS | set(CODEX_PARENT_DIR_NAMES))
                    ):
                        continue
