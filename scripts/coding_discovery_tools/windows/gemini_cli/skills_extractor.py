"""
Gemini CLI skills extraction for Windows systems.

Extracts Gemini CLI skills from all projects, grouping them by project root.

Global skills:  ~/.gemini/skills/<name>/SKILL.md (+ ~/.agents/skills)
...
from ...gemini_cli_skills_helpers import (
...
class WindowsGeminiCliSkillsExtractor(BaseGeminiCliSkillsExtractor):
    """Extractor for Gemini CLI skills on Windows systems."""
    ...
    def extract_all_skills(self) -> Dict:
        """
        Extract all Gemini CLI skills from all projects on Windows.
        ...
        logger.info(f"Searching for Gemini CLI skills from root: {root_path}")
    ...
    def _extract_user_level_skills(self, user_skills: List[Dict]) -> None:
        """Extract user-level skills from ~/.gemini/skills/ for each user."""
        def extract_for_user(user_home: Path) -> None:
            extract_gemini_cli_user_level_items(user_home, user_skills, extract_single_rule_file, GEMINI_CLI_ITEM_CONFIGS)
            ...
            for item in current_dir.iterdir():
                try:
                    if (
                        ...
                        or traverses_other_tool_config_dir(item, allow=SHARED_SKILL_DIRS | set(GEMINI_CLI_PARENT_DIR_NAMES))
                    ):
                        continue
