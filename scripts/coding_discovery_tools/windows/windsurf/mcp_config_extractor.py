"""
MCP config extraction for Windsurf on Windows systems.
"""
...
class WindowsWindsurfMCPConfigExtractor(BaseMCPConfigExtractor):
    ...
    def extract_mcp_config(self) -> Optional[Dict]:
        """
        Extract Windsurf MCP configuration on Windows.
    ...
    def _extract_global_config(self) -> Optional[Dict]:
        r"""Extract global MCP config from ~/.windsurf/mcp_config.json"""
        ...
        try:
            ...
            if mcp_servers_array:
                # Use the actual path of the global config file's parent directory (Windsurf directory)
    ...
    def _extract_project_level_configs(self) -> List[Dict]:
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                futures = {
                    executor.submit(
                        self._walk_for_windsurf_mcp_configs,
                        root_path, dir_path, global_windsurf_dir, current_depth=1
                    )
            ...
            try:
                global_windsurf_dir = self.GLOBAL_MCP_CONFIG_PATH.parent
                walk_for_windsurf_mcp_configs(
                    root_path, root_path, projects, global_windsurf_dir,
    ...
    def _walk_for_windsurf_mcp_configs(
        self,
        root_path: Path,
        current_dir: Path,
        ...
        """
        Walk directory tree looking for .windsurf/mcp_config.json files.
        ...
        walk_for_windsurf_mcp_configs(
            root_path, current_dir, projects, global_windsurf_dir,
