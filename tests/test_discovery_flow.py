...
class TestNoToolsSentryEvent(unittest.TestCase):
    ...
    @unittest.skipUnless(os.name == "posix", "POSIX signal handling")
    def test_install_surfaces_ride_the_zero_tool_event(self):
        mock_sentry = Mock()
        self._run_main([], mock_sentry)

        ctx = self._context_of(next(
            c for c in mock_sentry.call_args_list
            if self._context_of(c).get("phase") == "no_tools_found"
        ))
        self.assertIsInstance(ctx.get("install_surfaces"), dict)
        self.assertIsInstance(ctx.get("install_surfaces_total"), int)
        self.assertIs(ctx.get("install_surfaces_truncated"), False)

    def test_surface_scalars_are_taggable_but_the_listing_is_not(self):
        """Entry names are unbounded cardinality: tagging them would blow up the
        tag store, so only the two scalars may be promoted."""
        import scripts.coding_discovery_tools.utils as u
        self.assertIn("install_surfaces_total", u._SENTRY_TAG_KEYS)
        self.assertIn("install_surfaces_truncated", u._SENTRY_TAG_KEYS)
        self.assertNotIn("install_surfaces", u._SENTRY_TAG_KEYS)


class TestInstallSurfaceListing(unittest.TestCase):
    """Every detector gates on a hard-coded path, so a zero-tool scan cannot say
    whether the box is empty or the app is sitting somewhere we never named."""

    def test_caps_names_but_still_counts_them(self):
        import scripts.coding_discovery_tools.utils as u

        with tempfile.TemporaryDirectory() as tmp:
            for i in range(500):
                os.mkdir(os.path.join(tmp, f"app{i:03d}"))
            with patch.object(u.platform, "system", return_value="Windows"), \
                    patch.dict(os.environ, {"ProgramW6432": tmp}, clear=False):
                surfaces, total, truncated = u.install_surface_listing([])

        surface = surfaces["ProgramFiles"]
        self.assertEqual(surface["state"], "present")
        # The count is the truth about the directory; the names are the sample.
        self.assertEqual(surface["n"], 500)
        self.assertEqual(len(surface["names"]), u._SURFACE_MAX_NAMES)
        self.assertEqual(total, 500)
        self.assertIs(truncated, True)

    def test_absent_is_not_reported_as_empty(self):
        import scripts.coding_discovery_tools.utils as u

        missing = os.path.join(tempfile.gettempdir(), "definitely-not-here-xyz")
        with patch.object(u.platform, "system", return_value="Windows"), \
                patch.dict(os.environ, {"ProgramW6432": missing}, clear=False):
            surfaces, total, truncated = u.install_surface_listing([])

        self.assertEqual(surfaces["ProgramFiles"]["state"], "absent")
        self.assertEqual(total, 0)
        self.assertIs(truncated, False)

    def test_per_user_programs_is_listed_beside_its_parent(self):
        """Cursor/Windsurf/VS Code install one level inside Programs, so listing
        only LocalAppData reports a single entry named 'Programs'."""
        import scripts.coding_discovery_tools.utils as u

        with tempfile.TemporaryDirectory() as home:
            programs = os.path.join(home, "AppData", "Local", "Programs")
            os.makedirs(programs)
            os.mkdir(os.path.join(programs, "Cursor"))
            with patch.object(u.platform, "system", return_value="Windows"), \
                    patch.dict(os.environ, {}, clear=True):
                surfaces, _, _ = u.install_surface_listing([home])

        self.assertEqual(surfaces["LocalAppData"]["names"], ["Programs"])
        self.assertEqual(surfaces["LocalAppData\\Programs"]["names"], ["Cursor"])

    def test_never_raises(self):
        import scripts.coding_discovery_tools.utils as u

        with patch.object(u.os, "scandir", side_effect=OSError("denied")), \
                patch.object(u.platform, "system", return_value="Darwin"):
            surfaces, total, truncated = u.install_surface_listing(["/nonexistent-home"])
        self.assertEqual(surfaces["/Applications"]["state"], "unreadable")
        self.assertEqual(total, 0)
