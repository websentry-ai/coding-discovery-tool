...
Driven against a stub stdio MCP server so nothing depends on npx reaching the registry.
"""
...
class TestStdioEmbeddedArgs(unittest.TestCase):
    ...
    def setUp(self):
        ...
        spaced_dir.mkdir()
        self.spaced_exe = spaced_dir / "run me"
        self._write_stub(self.spaced_exe)
    ...
    def test_path_with_spaces_never_reaches_the_fallback(self):
        ...
        with patch.object(scanner.shlex, "split", side_effect=AssertionError("must not split")) as spy:
            result = scanner._scan_stdio(str(self.spaced_exe), ["--flag"], {}, 10)
